...
class WPCustomerReviews3 {
	var $debug = false;
	var $prefix = 'wpcr3';
	var $dashname = 'wp-customer-reviews';
	...
	function include_goatee() {
		if (!defined('HAS_'.$this->prefix.'_GOATEE')) {
			...
			include_once($this->getplugindir() . 'include/goatee-php/wpcr-goatee.php'); 
		}
	}
	...
	function include_admin() {
		if (!defined('HAS_'.$this->prefix.'_ADMIN')) {
			...
			include_once($this->getplugindir() . 'include/admin/wp-customer-reviews-3-admin.php'); 
	...
	function ajax() {
		...
		if ($posted->ajaxAct === 'cookie') {
			...
			if (count($rtn->err)) { die(json_encode($rtn)); } 
	...
	function setSharedVars($parentClass) {
		$this->plugin_info = &$parentClass->plugin_info; 
    ...
    function init() {
		...
		add_shortcode('WPCR_HCARD', array(&$this, 'shortcode_hcard')); 
    ...
    function shortcode_show($atts) {
        ...
        if (strtolower($opts->postid) == 'all') { $opts->postid = -1; } 
		...
		if ($opts->postid === -1) { $opts->showform = 0; } 
		$opts->showsupport = 0;
    ...
    function activate() {
        add_option($this->prefix.'_gotosettings', true); 
    }
